 ...
 */
contract BizflipAuction is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event BizflipAuctionContractDeployed();
...
function initialize(address payable _platformFeeRecipient, address _owner)
    public
    initializer
{
    require(
        _platformFeeRecipient != address(0),
        "BizflipAuction: Invalid Platform Fee Recipient"
    );

    platformFeeRecipient = _platformFeeRecipient;
    emit BizflipAuctionContractDeployed();

     __Ownable_init(); // Initialize Ownable contract without arguments
    __ReentrancyGuard_init(); // Initialize ReentrancyGuard (no arguments required for ReentrancyGuard_init)
}
 ...
 */
  ...
  function placeBid(
    ...
    require(!_msgSender().isContract(), "no contracts permitted");
    ...
    nonReentrant
{
    ...
    IBizflipMarketplace marketplace = IBizflipMarketplace(
    ...
    emit AuctionResulted(
        ...
        winner,
        auction.payToken,
        IBizflipMarketplace(addressRegistry.marketplace()).getPrice(
            auction.payToken
        ),
        winningBid
    );
