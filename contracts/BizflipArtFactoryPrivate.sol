...
import "./BizflipArtTradablePrivate.sol";

contract BizflipArtFactoryPrivate is Ownable {
    /// @dev Events of the contract
    event ContractCreated(address creator, address nft);
    event ContractDisabled(address caller, address nft);

    /// @notice Bizflip marketplace contract address;
    address public marketplace;

    /// @notice Bizflip bundle marketplace contract address;
    ...
    */
    ...
    /// @notice Method for deploy new BizflipArtTradablePrivate contract
    /// @param _name Name of NFT contract
    /// @param _symbol Symbol of NFT contract
    /// @param additionalArg An additional argument required by the BizflipArtTradablePrivate constructor
    function createNFTContract(
        string memory _name,
        string memory _symbol,
        address additionalArg // Add the missing argument here
    ) external payable returns (address) {
        require(msg.value >= platformFee, "Insufficient funds.");
        (bool success, ) = feeRecipient.call{value: msg.value}("");
        require(success, "Transfer failed");

        BizflipArtTradablePrivate nft = new BizflipArtTradablePrivate(
            _name,
            _symbol,
            mintFee,
            feeRecipient,
            marketplace,
            bundleMarketplace,
            additionalArg // Pass the additional argument here
        );
    ...
    /// @notice Method for registering existing BizflipArtTradable contract
    ...
    /// @notice Method for disabling existing BizflipArtTradable contract
    /// @param  tokenContractAddress Address of NFT contract
