...
contract BizflipBundleMarketplace is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    initializer
{
    ...
    // Only one definition of _getBundleID function
   function _calculateBundleID(string memory _bundleID) private pure returns (bytes32) {
        return keccak256(abi.encodePacked(_bundleID));
    }

    function getBundleID(string memory bundleID) private pure returns (bytes32) {
        return keccak256(abi.encodePacked(bundleID));
    }
        ...
        require(
            _payToken == address(0) ||
                (tokenRegistry != address(0) &&
                    IBizflipTokenRegistry(tokenRegistry).enabled(_payToken)),
            "invalid pay token"
        );
    ...
    function _buyItem(string memory _bundleID, address _payToken) private {
        ...
        if (_payToken == address(0)) {
            ...
            require(
                feeTransferSuccess,
                "BizflipMarketplace: Fee transfer failed"
            );
            ...
            require(
                ...
                "BizflipMarketplace: Owner transfer failed"
            );
        } else {
        ...
        for (uint256 i; i < listing.nfts.length; i++) {
            ...
            IBizflipMarketplace(addressRegistry.marketplace()).validateItemSold(
                listing.nfts[i],
                listing.tokenIds[i],
                owner,
                _msgSender()
        ...
        emit ItemSold(
            ...
            _bundleID,
            _payToken,
            IBizflipMarketplace(addressRegistry.marketplace()).getPrice(_payToken),
            price
        );
        ...
        nonReentrant
    {
        ...
        uint256 feeAmount = div(mul(price, platformFee), 1e3);
        ...
        emit ItemSold(
            ...
            IBizflipMarketplace(addressRegistry.marketplace()).getPrice(address(offer.payToken)),
            offer.price
        );
    ...
    function validateItemSold(
        ...
        for (uint256 i; i < length; i++) {
            ...
            // Use the subtraction operator directly
                    listing.quantities[index] -= _quantity;
                } else {
